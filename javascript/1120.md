...
**Input**
The input consists of several test cases, each in one line. Each line contains two integers D and N (1 ≤ D ≤ 9, 1 ≤ N < 10^100), representing, respectively, the digit that has failed in the machine and the number that was originally agreed for the contract (which can be very large because of hiperinflation).
...
while(true)
{
    ...
    let result = N.replace(new RegExp(D, 'g'), '');
    ...
    if(result === '' || result === '0' || /^0+$/.test(result)){
        ...
        let finalNumber = result.replace(/^0+/, '');
